...
# Diretório para dados históricos
DATA_DIR = 'dados'
...
app = FastAPI(
    ...
    description="Uma API RESTful com dashboard para visualização de métricas",
    version="6.0.0"
)
...
# Histórico de previsões para o dashboard
historico_previsoes = []
...
if os.path.exists(HISTORY_FILE):
    try:
        with open(HISTORY_FILE, 'r') as f:
            historico_previsoes = json.load(f)
        logger.info(f"Carregado histórico com {len(historico_previsoes)} previsões")
...
def salvar_historico():
    try:
        with open(HISTORY_FILE, 'w') as f:
            json.dump(historico_previsoes, f)
        logger.info(f"Histórico salvo com {len(historico_previsoes)} previsões")
    except Exception as e:
...
def index():
    ...
    return {"mensagem": "API de Previsão de Preços de Imóveis v6.0 com Dashboard"}
...
def prever(dados_processados: Tuple[np.ndarray, List[str], Dict, str] = Depends(preparar_dados_previsao)):
    ...
    try:
        ...
        # Grava no histórico para o dashboard
        timestamp = datetime.now().isoformat()
        historico_previsoes.append({
            ...
            "faixa_confianca": faixa_confianca
        })
        
        # Limita o histórico a 1000 previsões
        if len(historico_previsoes) > 1000:
            historico_previsoes = historico_previsoes[-1000:]
...
# Rota para obter todas as previsões (para o dashboard)
@app.get("/previsoes", response_model=List[HistoricoPrevisao])
def listar_previsoes(limit: int = 100):
    # Retorna as últimas N previsões para o dashboard
    return historico_previsoes[-limit:]
...
def status(request: Request):
    ...
    estatisticas_atualizadas = {
        **api_stats,
        ...
        "total_previsoes_historico": len(historico_previsoes)
    }
